...
function get_stray_quotes($categories=NULL,$sequence=NULL,$linkphrase=NULL,$multi=NULL,$timer=NULL,$noajax=NULL,$myoffset=0,$widgetid=NULL,$fullpage=NULL,$orderby='quoteID',$sort='ASC',$thisid=NULL,$disableaspect=NULL,$contributor=NULL) {
	...
	if (is_string($multi) || is_bool($multi)) {
		settype($multi, "integer");
	}
	if ($multi == 0 || $multi == '' || false == $multi) {
		$multi = 1;
	}
	if (is_string($timer)) {
		settype($timer, "integer");
	}
	if ($timer == '' || false === $timer) {
		$timer = 0;
	}
	if (is_string($noajax)) {
		if ($noajax == 'false') {
		...
		if ($fullpage){
			...
			for($quotepage = 1; $quotepage <= $maxPage; $quotepage++) {
				if ($quotepage == $pages) {
					...
					$nav .= ' <a href="' . $urlpages . $quotepage . '">' . $quotepage . '</a> ';
				}
			}
	...
	if ( !empty($result) ){
		...
		if ($strayajax != 'Y' && $noajax != true ) {
			...
			if (!$quoteloader && $timer < 2) {
				$output .= '<div class="stray_quote-' . $widgetid . '" ' . $event . ' >';
			} else {
				$output .= '<div class="stray_quote-' . $widgetid . '">';
			}
		}
		...
		//output if one quote only
			...
			//if a specific quote
			if ($thisid && $thisid != '') {
				$output .= stray_output_one($specificresult, $multi, $disableaspect);
			}
			...
			//if it is not a specific quote
				...
				if (is_numeric($sequence) || $sequence === '0') {
					...
					$get_one = $result[mt_rand(0, $totalquotes)];
				}
		...
		if ($strayajax != 'Y' && $noajax != true ) {
			...
			//timeout thing
			if ($timer) {
				$output .= '<script type="text/javascript">setTimeout("' . $jaction . '", "' . $timer . '000");</script>';
			}
		}
...
//this FORMATS a CPT quote (WP_Post object) according to the settings
