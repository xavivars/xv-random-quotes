...
edButtons[ edButtons.length ] = new edButton( 'ed_link', 'link', '', '</a>' ); // special case
	...
	if ( button.id == 'ed_link' ) {
		document.write(
			'<input type="button" id="' +
				button.id +
				'" class="ed_button" onclick="edInsertLink(edCanvas, ' +
				i +
				');" value="' +
				button.display +
				'" />'
		...
		document.write(
			'<input type="button" id="' +
				button.id +
				'" class="ed_button" onclick="edInsertTag(edCanvas, ' +
				i +
				');" value="' +
				button.display +
				'" />'
...
function edShowButton1( button, i ) {
	if ( button.id == 'ed_link' ) {
		document.write(
			'<input type="button" id="' +
				button.id +
				'" class="ed_button" onclick="edInsertLink(edCanvas1, ' +
				i +
				');" value="' +
				button.display +
				'" />'
		...
		document.write(
			'<input type="button" id="' +
				button.id +
				'" class="ed_button" onclick="edInsertTag(edCanvas1, ' +
				i +
				');" value="' +
				button.display +
				'" />'
...
function edShowButton2( button, i ) {
	if ( button.id == 'ed_link' ) {
		document.write(
			'<input type="button" id="' +
				button.id +
				'" class="ed_button" onclick="edInsertLink(edCanvas2, ' +
				i +
				');" value="' +
				button.display +
				'" />'
		...
		document.write(
			'<input type="button" id="' +
				button.id +
				'" class="ed_button" onclick="edInsertTag(edCanvas2, ' +
				i +
				');" value="' +
				button.display +
				'" />'
...
function edRemoveTag( button ) {
	for ( i = 0; i < edOpenTags.length; i++ ) {
		if ( edOpenTags[ i ] == button ) {
			...
			document.getElementById( edButtons[ button ].id ).value = document
				.getElementById( edButtons[ button ].id )
				.value.replace( '/', '' );
		}
	}
}
...
function edInsertTag( myField, i ) {
	...
	if ( document.selection ) {
		...
		if ( sel.text.length > 0 ) {
			sel.text =
				edButtons[ i ].tagStart + sel.text + edButtons[ i ].tagEnd;
		} else {
	...
	//MOZILLA/NETSCAPE support
		...
		if ( startPos != endPos ) {
			...
			cursorPos +=
				edButtons[ i ].tagStart.length + edButtons[ i ].tagEnd.length;
		} else {
			if ( ! edCheckOpenTags( i ) || edButtons[ i ].tagEnd == '' ) {
				myField.value =
					myField.value.substring( 0, startPos ) +
